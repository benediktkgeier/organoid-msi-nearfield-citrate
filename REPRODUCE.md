...
the manual-gate reference choices, and result tables). The **raw data** needed to reproduce the study
is publicly deposited (below). The intermediate **cache** (`peaks_*`, `citrate_anchored_*`,
`instances_*`, `zones_*`, `register*/`) and the rendered **figures** are **derived** artifacts the
pipeline regenerates from that raw data — they are `.gitignore`d, not a required separate download.
...
>
> **📦 Intermediate cache + rendered figures — derived, not a separate download.** The validated
> `cache/` tree and the figure PDFs are **regenerated by the pipeline** from the public raw data above.
> Downloading the **METASPACE imzML** (+ figshare `.nd2`) is sufficient to reproduce: run **Mode B** to
> rebuild the cache from raw, then the reports. (Our exact cache is also available from the authors on
> request, to skip the rebuild — place it at `<MSI_ROOT>/Analysis_R/cache`, see §2–§3.)

*(All raw data required to reproduce is publicly available now — MSI imzML on METASPACE, microscopy and
Bruker raw on figshare. The cache and figures are pipeline outputs, not a required deposit.)*
...
|---|---|---|---|
| **A. Figure-layer (recommended)** | Regenerate every figure/result PDF + CSV from the validated intermediate **cache** | R env + Java + the `cache/` (built by Mode B from the public imzML, or from the authors on request) | ~15–40 min |
| **B. From-raw rebuild** | Re-run preprocessing → segmentation from the **public** raw imzML (METASPACE) + `.nd2` (figshare) | R env + Java + raw imzML/`.nd2` + **the committed manual-gate artifacts** | hours; **partial** (see §6) |
...
is the faithful reproduction target. **External reproducers** obtain the cache by running **Mode B** on
the public METASPACE imzML (+ figshare `.nd2`), then run Mode A to regenerate the figures — downloading
the METASPACE imzML is sufficient.
...
2. **Java 8 JRE** (Bio-Formats backend for `RBioFormats`, required by every `.nd2` step). Set
   `JAVA_HOME` to it (§3.3).
3. **Disk**: ~2 GB for the cache (some intermediate `.rds` are ~800 MB each; built by Mode B or
   supplied on request).
4. **Raw data** — see §2. Public: **METASPACE** imzML + **figshare** `.nd2`. The `cache/` is derived
   (build it via Mode B, or request our copy).
...
The pipeline reads **public raw acquisitions** (METASPACE imzML + figshare `.nd2`) and a set of
**derived** intermediate caches (built by Mode B, or supplied on request). Provide them in this layout
...
│  └─ R/ docs/ *.sh
├─ Analysis_R/cache/            # << derived cache — read-only upstream (Mode B builds it, or request our copy)
│  ├─ imzml/                    # MSI imzML + .ibd, all 20 sections (the effective MSI raw = METASPACE datasets)
