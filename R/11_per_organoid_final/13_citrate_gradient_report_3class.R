#!/usr/bin/env Rscript
# ============================================================================
# 13_citrate_gradient_report_3class.R - apical-CLASS variant of the final report.
#   SEPARATE, non-destructive sibling of 10_citrate_gradient_report_final.R (which
#   is left untouched). Same 6-panel per-dataset pages + overview; the ONLY change
#   is how the apical class "mixed" is treated in the overview trend lines:
#
#     DEFAULT (with mixed)  -> "citrate_gradient_report_withmixed.pdf"
#         mixed is a FULL third group: it gets its OWN bold trend line (grey) in
#         each overview mini, alongside apical-out (magenta) and basolateral-out
#         (green). Use this to show the 3-class comparison transparently.
#     `nomixed`             -> "citrate_gradient_report_nomixed.pdf"
#         mixed organoids are DROPPED entirely: 2-class view (apical-out vs
#         basolateral-out), mixed omitted from the minis, class outlines, and
#         legend. Use this as the clean headline figure.
#
#   The two together give both publication options. (10_..._final.pdf keeps its own
#   "mixed as grey context, no bold trend" treatment - unchanged.)
#   Shares the frame primitives (lib_report_frame.R) and the locked whole-region
#   toolkit; the 6 per-panel helpers mirror 10_ verbatim (kept in sync by hand).
#
# In : same as 10_ (peaks_tissue_combined.rds, zones_<sid>.rds, register/nd2final,
#      optical_<sid>.png, if_optical_<sid>.png, apical_map_consensus.csv)
# Out: figures/gradient/citrate_gradient_report_{withmixed,nomixed}[_TEST_<sid>].pdf
# Usage: Rscript R/11_per_organoid_final/13_citrate_gradient_report_3class.R [all|test|<sid>] [nomixed]
# ============================================================================
...
source(file.path(ROOT, "R/00_lib/lib_gradient_report.R"))       # shared per-dataset panels (sec_data + 6 panels), same as 10_
suppressPackageStartupMessages({ library(Cardinal); library(png); library(viridisLite); library(EBImage) })

log_msg <- function(...) message(sprintf("[3class] %s", sprintf(...)))
...
# ============================================================================
# SHARED SETUP (mirrors 10_ / 08 / 09)
# ============================================================================
...
ap_map  <- setNames(ap$apical_class, paste(ap$sid, ap$instance))   # full 3-class map

ord20 <- levels(pd$sample_id); if (is.null(ord20)) ord20 <- unique(as.character(pd$sample_id))
ord20 <- sort(ord20)

# Args: [all|test|<sid>] and an optional `nomixed` token (order-independent).
a <- commandArgs(trailingOnly = TRUE)
DROP_MIXED <- any(tolower(a) == "nomixed")
a2   <- a[tolower(a) != "nomixed"]
mode <- if (length(a2) == 0) "all" else tolower(a2[1])
...
} else                    { RENDER_SIDS <- a2[1];    test_mode <- TRUE }   # explicit <sid>
RENDER_SIDS <- RENDER_SIDS[RENDER_SIDS %in% ord20]
stopifnot(length(RENDER_SIDS) >= 1)

# de-emphasise mixed in the spatial panels when nomixed (mixed -> unannotated grey70)
ap_map_use    <- if (DROP_MIXED) ap_map[ap_map != "mixed"] else ap_map
# bold trend lines drawn per overview mini
TREND_CLASSES <- if (DROP_MIXED) c("basolateral_out","apical_out") else c("basolateral_out","apical_out","mixed")

VARIANT <- if (DROP_MIXED) "nomixed" else "withmixed"
OUT_PDF <- if (nzchar(Sys.getenv("FINAL_OUT"))) file.path(GRAD_FIG, Sys.getenv("FINAL_OUT")) else
  if (test_mode)
  file.path(GRAD_FIG, sprintf("citrate_gradient_report_%s_TEST_%s.pdf", VARIANT, RENDER_SIDS[1])) else
  file.path(GRAD_FIG, sprintf("citrate_gradient_report_%s.pdf", VARIANT))

HEAT_HI <- heatmap_global_clip(ord20)     # pooled global p99.9 (comparable page-to-page)
log_msg("variant=%s | HI(p99.5)=%.3g | HEAT_HI=%.3g", VARIANT, HI, HEAT_HI)
...
# ============================================================================
# OVERVIEW mini plot - THE apical-class variant: mixed gets its own bold trend
# (unless DROP_MIXED, then mixed organoids are excluded entirely).
# ============================================================================
class_trend_mini <- function(sid) {
  ...
  zsel <- which(OUT_ZONE_UM <= OV_MAX_UM); nz <- length(zsel)
  prof_k <- function(k) {
  ...
  if (DROP_MIXED) { keep <- cls != "mixed"; ids <- ids[keep]; cls <- cls[keep] }   # drop mixed organoids
  ...
  for (cc in TREND_CLASSES) {                                   # apical/baso (+ mixed grey unless nomixed)
...
dataset_page <- function(sid) {
  sd  <- sec_data(sid); sec <- prep_sec(sid); z <- sec$z
  inst_ids <- if (is.null(z)) integer(0) else sort(unique(z$instance[z$instance > 0]))
  n_annot  <- sum(!is.na(ap_map_use[paste(sid, inst_ids)]))
  ...
  native_overlay_norings(sec, sid, ap_map_use, "5) citrate on brightfield (overlay)",
                         "citrate on BF - a0.60 - class outlines")
  weather_panel(sec, sid, ap_map_use, "6) citrate gradient map (weather, GLOBAL)",
                "weather - gblur 12um - global p99.9")
  mtext(sprintf("%s   -   %d organoid%s (%d annotated)%s", disp_id(sid), length(inst_ids),
                if (length(inst_ids)==1) "" else "s", n_annot, if (DROP_MIXED) ", mixed excluded" else ""),
        outer = TRUE, line = 3.1, cex = 1.6, font = 2)
  ...
  if (DROP_MIXED) {
    legend_cols <- c(CCOL_WHOLE[["basolateral_out"]], CCOL_WHOLE[["apical_out"]], UNANNOT_COL)
    legend_lab  <- c("basolateral-out","apical-out","unannotated")
  } else {
    legend_cols <- c(CCOL_WHOLE[["basolateral_out"]], CCOL_WHOLE[["apical_out"]], CCOL_WHOLE[["mixed"]], UNANNOT_COL)
    legend_lab  <- c("basolateral-out","apical-out","mixed","unannotated")
  }
  ...
  legend("bottom", horiz = TRUE, bty = "n", cex = 0.8, lwd = 3, inset = c(0, 0.003),
         legend = legend_lab, col = legend_cols, text.col = "grey15")
}
...
overview_page <- function() {
  ...
  for (sid in ord20) {
    sec <- prep_sec(sid)
    draw_gradmap_whole(sec, sid, ap_map_use, disp_id(sid))
    class_trend_mini(sid)
  }
  ttl <- if (DROP_MIXED) "OVERVIEW (nomixed) - all 20 datasets: citrate ion image + 50/100 um rings + per-organoid outward curves"
         else            "OVERVIEW (with mixed) - all 20 datasets: citrate ion image + 50/100 um rings + per-organoid outward curves"
  sub <- if (DROP_MIXED) "curves normalized to organoid surface (=1.0); faint=organoids by class; BOLD=class mean; MAGENTA=apical-out, GREEN=basolateral-out; mixed EXCLUDED; x='in'+outward zones (um)"
         else            "curves normalized to organoid surface (=1.0); faint=organoids by class; BOLD=class mean; MAGENTA=apical-out, GREEN=basolateral-out, GREY=mixed (3rd group); x='in'+outward zones (um)"
  mtext(ttl, outer = TRUE, line = 2.7, cex = 1.3, font = 2)
  mtext(sub, outer = TRUE, line = 1.0, cex = 0.82, col = "grey25")
}
